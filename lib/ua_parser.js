module.exports = function useragent_parser(ua) {
  ...
  if (!ua) {
		...
		minor = result[3];
		patch = result[4];
	} else if (result = /\b(008|Altresium|Argus|BaiduMobaider|BoardReader|DNSGroup|DataparkSearch|EDI|Goodzer|Grub|INGRID|Infohelfer|LinkedInBot|LOOQ|Nutch|PathDefender|Peew|PostPost|Steeler|Twitterbot|VSE|WebCrunch|WebZIP|Y!J-BR[A-Z]|YahooSeeker|envolk|sproose|wminer)\/(\d+)(?:\.(\d+)(?:\.(\d+))?)?/.exec(ua)) {
		family = result[1];
		major = result[2];
		minor = result[3];
		patch = result[4];
	} else if (result = /(MSIE) (\d+)\.(\d+)([a-z]\d?)?;.* MSIECrawler/.exec(ua)) {
		family = "MSIECrawler";
		major = result[2];
		minor = result[3];
		patch = result[4];
		...
		minor = result[3];
		patch = result[4];
	} else if (result = /(Google-HTTP-Java-Client|Apache-HttpClient|Go-http-client|scalaj-http|http%20client|Python-urllib|HttpMonitor|TLSProber|WinHTTP|JNLP|okhttp|aihttp|reqwest)(?:[ \/](\d+)(?:\.(\d+)(?:\.(\d+))?)?)?/.exec(ua)) {
		family = result[1];
		major = result[2];
		minor = result[3];
		patch = result[4];
		...
		minor = result[3];
		patch = result[4];
	} else if (result = /((?:[A-z0-9]+|[A-z\-]+ ?)?(?: the )?(?:[Ss][Pp][Ii][Dd][Ee][Rr]|[Ss]crape|[A-Za-z0-9-]*(?:[^C][^Uu])[Bb]ot|[Cc][Rr][Aa][Ww][Ll])[A-z0-9]*)(?:(?:[ \/]| v)(\d+)(?:\.(\d+)(?:\.(\d+))?)?)?/.exec(ua)) {
		family = result[1];
		major = result[2];
		minor = result[3];
		patch = result[4];
		...
		family = "Firefox ($1)".replace('$1', result[1]);
		major = result[2];
		minor = result[3];
		patch = result[4];
		...
		family = "Outlook";
		major = "2013";
	} else if (result = /Microsoft Outlook (?:Mail )?16\.\d+\.\d+/.exec(ua)) {
		family = "Outlook";
		major = "2016";
		...
		minor = result[3];
		patch = result[4];
	} else if (result = /(Edge)\/(\d+)(?:\.(\d+))?/.exec(ua)) {
		family = result[1];
		major = result[2];
		minor = result[3];
		...
		major = result[2];
		minor = result[3];
	} else if (result = /\b(Windows-Update-Agent|Microsoft-CryptoAPI|SophosUpdateManager|SophosAgent|Debian APT-HTTP|Ubuntu APT-HTTP|libcurl-agent|libwww-perl|urlgrabber|curl|PycURL|Wget|aria2|Axel|OpenBSD ftp|lftp|jupdate|insomnia)(?:[ \/](\d+)(?:\.(\d+)(?:\.(\d+))?)?)?/.exec(ua)) {
		family = result[1];
		major = result[2];
		minor = result[3];
		patch = result[4];
